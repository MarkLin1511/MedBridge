...
function StatusBadge({ status }: { status: string }) {
  if (status === "high") {
    return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 dark:bg-red-900/40 text-red-700 dark:text-red-300">High</span>;
  }
  if (status === "low") {
    return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300">Low</span>;
  }
  return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">Normal</span>;
}

function TrendIcon({ trend }: { trend: string }) {
  if (trend === "up") return <span className="text-emerald-500 text-lg">↑</span>;
  if (trend === "down") return <span className="text-teal-500 text-lg">↓</span>;
...
function SourceBadge({ source }: { source: string }) {
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors}`}>{source}</span>;
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-20 md:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        {/* Demo banner */}
        <FadeIn>
          <div className="mb-6 bg-teal-50 dark:bg-teal-950/30 border border-teal-200 dark:border-teal-800 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                DOB: {patient.dob} &middot; ID: {patient.id} &middot; {patient.wearable}
              </p>
            </div>
            <div className="flex gap-2 flex-wrap">
              {patient.connectedPortals.map((p) => (
                <span key={p} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 text-xs font-medium">
                ...
                <tbody>
                  {labResults.map((lab) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-gray-400 font-mono text-xs hidden sm:table-cell">{lab.loinc}</td>
                      <td className="px-5 py-3.5 text-gray-900 dark:text-white font-medium">{lab.value} {lab.unit}</td>
                      <td className="px-5 py-3.5 text-gray-400 hidden md:table-cell">{lab.range}</td>
                      <td className="px-5 py-3.5"><StatusBadge status={lab.status} /></td>
                      <td className="px-5 py-3.5 text-gray-400 hidden lg:table-cell">{lab.date}</td>
        ...
        <FadeIn delay={0.1}>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl divide-y divide-gray-100 dark:divide-gray-800">
            {auditLog.map((entry, i) => (
              <div key={i} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <div>
                  <span className="text-sm text-gray-900 dark:text-white">{entry.action}</span>
                  <span className="text-sm text-gray-500 ml-1">by {entry.by}</span>
                </div>
